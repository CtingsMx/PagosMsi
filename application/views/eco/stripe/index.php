    ...
    <div class="col-md-5">
      <div id="details">
        ...
        <form id="payment-form">
          ...
          <button id="card-button">Pagar</button>
        </form>
      </div>
      ...
      <div id="plans" hidden>
        <form id="installment-plan-form">
          <label><input id="immediate-plan" type="radio" name="installment_plan" value="-1" />Un pago</label>
...
<script>
  ...
  form.addEventListener('submit', function(ev) {
    ...
    stripe.createPaymentMethod({
      ...
      if (result.error) {
        ...
        fetch('<?= base_url() ?>index.php/stripe/collect_details', {
          method: 'POST',
          headers: {
  ...
  confirmButton.addEventListener('click', async (ev) => {
    ...
    const response = await fetch('<?= base_url() ?>index.php/stripe/confirmarPago', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },

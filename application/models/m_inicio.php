...
class m_inicio extends CI_Model
{
    ...
    /**
     * Regresa todos los pedidos que estan en proceso de entrega
     * 
     * @return array
     */
    function pedidosAbiertos()
    {
        ...
        f.rfc as frfc,
        f.correo as fcorreo,
        f.direccion as fdireccion,
        f.uso as fuso,
        f.enviada as fenviada
        FROM tb_pedidos
        LEFT JOIN tb_datos_factura f ON pedido = tb_pedidos.id_pedido
        LEFT JOIN tb_clientes c ON tb_pedidos.cliente = c.id
        LEFT JOIN tb_estatusPedido s ON tb_pedidos.estatus = s.id 
        WHERE tb_pedidos.estatus < 4";
